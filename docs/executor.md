...
The executor runs the model and publishes the commitments needed to check it. This guide follows **DCG unified document format v1, specification revision 6**.
...
This is a testnet alpha. The program address and SDK calls are placeholders in the [quickstart](quickstart.md).
...
- the request ID, consumer digest, prompt, output count, seed, and DDT1 match the request;
- the selected output write has the declared fixed width;
- the executor has enough testnet lamports for rent, the executor bond, and fees.

A predictable DCG account may already hold lamports. Revision 6 lets the creating instruction top it up to rent. A pre-funded address is not a failure.
...
- `DDT1`: challenge window, response window, challenger bond, executor bond, and reward share;
- `DRB1`: executor, optional request binding, seed, and output locator.

Both blocks are hashed into the descriptor. A later edit changes every document address.
...
The executor declares the full response size in tag **115**. That stored size governs the response. A challenger's shorter `response_len` is advisory.
...
Tag **131, `Settle` on a DCR1 v5 record** uses:
...
It pays the record bond, may pay the executor-bond reward on the first settled challenger win, and returns the closed record's remaining rent to the challenger.
...
After the allowed deadline with no open challenge, anyone may close. The executor receives the document rent and any held executor bond. DCR2 remains as the durable result.
...
- **Terms refusal:** a mechanical DDT1 floor or cap fails. Zero policy bonds are valid.
