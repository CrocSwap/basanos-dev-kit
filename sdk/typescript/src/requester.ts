...
export class RequestBlock {
  ...
  static create(requestProgram: KeyLike, requester: KeyLike, options: RequestBlockOptions): RequestBlock {
...
export const Request = RequestBlock;
...
export class Requester {
  ...
  request(requestProgram: KeyLike, requester: KeyLike, options: RequestBlockOptions): RequestBlock {
    return RequestBlock.create(requestProgram, requester, options);
  }

  documentPlan(request: RequestBlock, accounts: DocumentAccounts, spec: c.DescriptorSpec, familyBody: Uint8Array, options: { positionRoots?: readonly Uint8Array[]; familyRoots?: readonly Uint8Array[]; landBatchSize?: number } = {}): DocumentPlan {
    const positionRoots = options.positionRoots ?? [];
    const familyRoots = options.familyRoots ?? [];
    if (!spec.binding.requestId.equals(request.request) || !spec.binding.consumerDigest.equals(request.consumerDigest()) || !sameTerms(spec.terms, request.terms) || !spec.binding.executor.equals(accounts.executor.toBytes()) || !c.buffer(familyBody).equals(spec.familyBody)) throw new Error("descriptor binding does not answer request");
  ...
  document_plan(request: RequestBlock, accounts: DocumentAccounts, spec: c.DescriptorSpec, familyBody: Uint8Array, options: { positionRoots?: readonly Uint8Array[]; familyRoots?: readonly Uint8Array[]; landBatchSize?: number } = {}): DocumentPlan {
...
function sameTerms(left: c.DisputeTerms, right: c.DisputeTerms): boolean {
  const values = (term: c.DisputeTerms): bigint[] => [term.challengeWindowSlots, term.responseWindowSlots, term.challengerBondLamports, term.executorBondLamports, term.executorRewardBps].map((value) => BigInt(value));
